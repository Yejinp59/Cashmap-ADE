...
| B4 | **live 모드 어댑터 불완전** | ✅ **오버레이 방식으로 재구현(7/4)** — 구 `_adaptLive`는 limit=1000(422)·`ds.score` 필드명 오류로 동작한 적 없었음. 신규 `_overlayLive`: 목업 데모 유지 + Supabase 실 D-Score 5개사(솔브레인 등)를 **'실데이터' 섹션 탭**으로 추가(피처는 실값 매핑, 괴리·공시는 데모 보강 명시). 배지도 3상태(실데이터 연결/서버 연결·데모/오프라인)로 정직화. 남은 것: 실 공시·괴리 연동은 AI-A 산출 대기 |
